...
#     cv2.imwrite(f"test{i}{j}.png", image)
#     return final_number
...
def read_sudoku_grid(result, sudoku_mode, matrix):
    ...
    for i in range(sudoku_mode):
        for j in range(sudoku_mode):
            ...
            region_of_interest = result[
                round((i * height / sudoku_mode)) : round(((i + 1) * height / sudoku_mode)),
...
def save_puzzle(size, digits):
    ...
    with open(Path("files/puzzle.txt"), "w") as file:
        for row in puzzle:
